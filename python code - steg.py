...
class StegInterface(wx.Frame):
 ...
 #self.picture.SetBitmap(wx.Bitmap('Lena.jpg'))
        self.image = wx.Image("placeholder.png", wx.BITMAP_TYPE_ANY)
        ...
        self.openFileBtn = wx.Button(self.panel1, wx.ID_ANY, "Open aFile", pos=(530, 400))
    ...
    def steganalyse(self, img):
        ...
        for row in range(height):
            for col in range(width):
                ...
                dict_colours = OrderedDict(sorted(list(colours_counter.items()),key=lambda t: t[1]))
 ...
 #dlg = wx.FileDialog(self, "Choose a file", os.getcwd(), "",wildcard, wx.FD_OPEN)
     ...
     #self.plainText.SetValue(dlg.GetPath())
            ...
            copyfile(self.imagePath, "lena.jpg")
            self.image.LoadFile("lena.jpg")
            img = self.image
            img.Rescale(500, 500)
